...
void OculusMobileDisplayPlugin::updatePresentPose() {
    ...
    VrHandler::withOvrMobile([&](ovrMobile* session){
        ...
        _currentPresentFrameInfo.predictedDisplayTime = vrapi_GetPredictedDisplayTime(session, _currentPresentFrameInfo.frameId);
        const auto trackingState = vrapi_GetPredictedTracking2(session, _currentRenderFrameInfo.predictedDisplayTime);
        _currentPresentFrameInfo.presentPose = ovr::toGlm(trackingState.HeadPose.Pose);
    });
}
...
static uint32_t vrPresentIndex { 0 };
...
void OculusMobileDisplayPlugin::destroyFrameInfo(FrameInfo& frameInfo) {
    if (frameInfo.extra) {
        delete (ovrTracking2*)frameInfo.extra;
        frameInfo.extra = nullptr;
    }
    Parent::destroyFrameInfo(frameInfo);
}
...
void OculusMobileDisplayPlugin::internalPresent() {
    ...
    if (_currentPresentFrameInfo.extra) {
        tracking = *((ovrTracking2*)_currentPresentFrameInfo.extra);
    } else {
        tracking = {};
        tracking.HeadPose.Pose.Orientation.w = 1.0f;
    }
    VrHandler::presentFrame(sourceTexture, sourceSize, tracking);
